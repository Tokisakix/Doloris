def define_label_binary(df):
    """
    二分类标签定义：
    - Pass 和 Distinction 记为 1（成功）
    - Fail 和 Withdrawn 记为 0（学业困难）

    :param df: 包含 'final_result' 列的 DataFrame
    :return: 增加 'label_binary' 列的 DataFrame
    """
    df = df.copy()
...
def define_label_multiclass(df):
    ...
    :param df: 包含 'final_result' 列的 DataFrame
    :return: 增加 'label_multiclass' 列的 DataFrame
    """
    label_map = {
        'Distinction': 2,
        'Pass': 1,
